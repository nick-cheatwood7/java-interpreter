...
public class Interpreter {
	...
	/*
	 * convertNameToInstance Given a name (identifier or literal) return the object.
	 * If the name is in the symbol table, you can just return the object it is
	 * associated with. If it's not in the symbol table, then it is either a String
	 * literal or it is an integer literal. Check to see if the first character is
	 * quotation marks. If so, create a new String object to return that has the
	 * same characters as the name, just without the quotes. If there aren't
	 * quotation marks, then it is an integer literal. Use Integer.parseInt to turn
	...
	/*
	 * convertNameToInstance. Takes an array of Strings and converts all of them to
	 * their associated objects. Simply call the other helper method of the same
	 * name on each item in the array.
	 */
	...
	/*
	 * makeObject This method does the "process" job for making objects. Don't
	...
	/*
	 * callMethod This method does the "process" job for calling methods. You MUST
	 * use the ReflectionUtilities to call the method for this to work, because ints
	 * can't be directly transformed into Objects. When you call a method, if it has
	 * an output, be sure to either create a new object for that output or change
	 * the existing object. This will require either adding a new thing to the
	 * symbol table or removing what is currently there and replacing it with
	 * something else.
	 */
	public String callMethod(ParseResults parse) {
		...
		if (mySymbolTable.containsKey(parse.answerName)) {
			...
			message = "I made a new object. Result was " + result;
		}
